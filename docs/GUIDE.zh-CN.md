...
- Apple Silicon Mac。原生应用最低目标为 macOS 14；不代表所有 MLX 版本都支持该系统。实际机器和 Laya 兼容情况见[验证记录](./VALIDATION.md)。
...
再次运行需使用新的输出名称。`artifacts/` 中保存完整合成输入／响应、共同请求和汇总，默认不进入 Git。评测包含 Top-1、Top-3、无匹配识别、候选召回、最近项和纯检索基线、耗时及超过产品两秒预算的比例。模型响应时间与从快捷键到粘贴完成的桌面耗时分别统计。实际证据与限制见[验证记录](./VALIDATION.md)。
...
- **Laya 快照缺失：** 重新运行 `download`。系统上的 Metal／MLX 失败应记录到[验证记录](./VALIDATION.md)，不能由构建成功推断兼容。
...
阅读[贡献指南](../CONTRIBUTING.md)、[规格](./SPEC.zh-CN.md)、[更新日志](../CHANGELOG.md)及[第三方说明](../THIRD_PARTY_NOTICES.md)。行为、安装方式或完成状态变化时同步两版 README。不要提交私人历史、真实凭据、模型权重或本地运行日志。
